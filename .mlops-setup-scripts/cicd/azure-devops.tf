...
resource "azurerm_role_definition" "role_definition" {
  name  = "cy-mlops_stack_ado-role-definition"
...
resource "azuredevops_variable_group" "cicd_vg" {
  ...
  variable {
    ...
    value = "https://adb-1352164841139608.8.azuredatabricks.net"
  }

  variable {
    name         = "STAGING-AZURE-SP-APPLICATION-ID"
    secret_value = module.create_staging_sp.service_principal_application_id
    is_secret    = true
  }

  variable {
    name         = "STAGING-AZURE-SP-CLIENT-SECRET"
    secret_value = module.create_staging_sp.service_principal_client_secret
    is_secret    = true
  }

  variable {
    # NOTE: assumes staging and prod Databricks workspaces are under the same Azure tenant
    name  = "STAGING-AZURE-SP-TENANT-ID"
  ...
  variable {
    name         = "PROD-AZURE-SP-APPLICATION-ID"
    secret_value = module.create_prod_sp.service_principal_application_id
    is_secret    = true
  }

  variable {
    name         = "PROD-AZURE-SP-CLIENT-SECRET"
    secret_value = module.create_prod_sp.service_principal_client_secret
    is_secret    = true
  }

  variable {
    name = "PROD-AZURE-SP-TENANT-ID"
    // NOTE: assumes staging and prod Databricks workspaces are under the same Azure tenant
    value = var.azure_tenant_id
  }
